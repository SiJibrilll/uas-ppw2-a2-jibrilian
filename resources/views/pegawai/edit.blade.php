        ...
        <div class="mx-auto max-w-screen-xl">
            <form action="{{ route('pegawai.update', ['id' => $data->id]) }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach ($pekerjaan as $item)
                            <option
                                value="{{ $item->id }}"
                                {{ (old('pekerjaan_id', $data->pekerjaan_id) == $item->id) ? 'selected' : '' }}
                            >
                                {{ $item->nama }}
                            </option>
                        @endforeach
                ...
                <div>
                    ...
                    <input type="text" name="email" value="{{ $data->email }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5" required>
                </div>
